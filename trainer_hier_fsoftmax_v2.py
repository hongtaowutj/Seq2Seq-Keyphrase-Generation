...
from models.hier_seq2seq_v2 import HierarchyFullSoftmax
...
def trainer(params):
	...
	try:
		...
		batch_train_iter = DataiteratorAll(X_train, y_train_in, y_train_out, vocab_size, decoder_dim=rnn_dim, batch_size=batch_size)

		val_steps = len(X_valid_pair)/batch_size
		batch_val_iter = DataiteratorAll(X_valid_pair, y_valid_in_pair, y_valid_out_pair, vocab_size, decoder_dim=rnn_dim, batch_size=batch_size)
	...
	sampled_softmax = HierarchyFullSoftmax(encoder_length, decoder_length, max_sents, embedding_dim, birnn_dim, rnn_dim, vocab_size, result_path, file_name, batch_train_iter, batch_val_iter, batch_size, steps_epoch, val_steps, epoch)
	...
	sampled_softmax.train_hier_seq2seq(pretrained_embedding, oov_embedding)
