...
class DecodingSoftmax():

	# input here is individual text
	def __init__(self, encoder_model, decoder_model, indices_words, words_indices, enc_in_seq, decoder_length, rnn_dim,  beam_width, num_hypotheses, filepath, filename):
		...
		for i in range(len(self.enc_in_seq)):
			...
			for t_ in range(self.decoder_length+1):
				...
				for n in fringe:
					...
					predicted_prob, state_t = self.decoder_model.predict([dec_input] + [states_value])
				...
				sys.stdout.flush()
				print("states shape: %s"%str(states.shape))
				sys.stdout.flush()
				print("predicted_indices shape: %s"%str(predicted_indices.shape))
				sys.stdout.flush()

				
				probs = probs.reshape((probs.shape[0],probs.shape[3]))
				states = states.reshape((states.shape[0],states.shape[2]))
				predicted_indices = predicted_indices.reshape((predicted_indices.shape[0],predicted_indices.shape[3]))
