...
class Decoding():
	...
	def __init__(self, encoder_model, decoder_model, indices_words, words_indices, enc_in_seq, labels, states, attentions, decoder_length, rnn_dim,  beam_width, num_hypotheses, filepath, filename):
	...
	def beam_decoder(self, texts, labels, s, att):
		...
		for i in range(len(self.enc_in_seq)):
			...
			for t_ in range(self.decoder_length+1):
				...
				for n in fringe:
					...
					label_t = labels[:, t_, :]
					predicted_prob, attention_t, state_t = self.decoder_model.predict([dec_input] + [encoder_output] + [states_value] + [label_t])
