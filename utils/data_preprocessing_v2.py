...
class Preprocessing():
	...
	def preprocess_in_generator(self):
		...
		for text_in in self.inputs:
			...
			text_in = re.sub(r'https?:\/\/\S+\b|[www\.]?(\w+\.\w+)+\S*', '<URL>', text_in)
	...
	def preprocess_in(self, input_text):
	...
	def preprocess_out_generator(self):
		...
		for kps in self.outputs:
			...
			prep_kps = []
			for kp in kps:
				kp = re.sub(r'https?:\/\/\S+\b|[www\.]?(\w+\.\w+)+\S*', '<URL>', kp)
	...
	def preprocess_out(self, output_keyphrases):
	...
	def tokenize_in(self, prep_inputs):
	...
	def tokenize_out(self, prep_outputs):
	...
	def get_all_tokens(self, inputs_tokens, output_tokens):
	...
	def all_tokens_sent_generator(self):

		for (in_tokens, out_tokens) in zip(self.inputs_tokens, self.output_tokens):
			tokens = []
			
			for sent in in_tokens:
				tokens.extend(sent)
	...
	def get_all_sent_tokens(self, inputs_tokens, output_tokens):

		self.inputs_tokens = inputs_tokens
		self.output_tokens = output_tokens

		all_tokens = []
		for tokens in self.all_tokens_sent_generator():
			all_tokens.extend(tokens)
			...
			for sent in doc:
				...
				if len(tokens) > 3:
					for t in tokens:
						...
						if (len(t) > 1) and (len(t) < 20):
							tokens_.append(t)
				if (len(tokens_) != 0) and (len(tokens_) > 3):
	...
	def tokenized_sent(self, input_text=None):
		...
		self.inputs_tokens = tokenized_docs

		return self.inputs_tokens
