...
class Predictioniterator():
	...
	def __init__(self, X, y_in, y_out, vocab_size=5000, decoder_dim=256, batch_size=32):
	...
	def __next__(self): # return model inputs - outputs per batch
		...
		batch_out = self.y_output[np.array(X_ids)]
		labels_one_hot = tf.one_hot(batch_out, self.num_classes) # one hot encoded of output 
		batch_y_output =  labels_one_hot # y_true values (model output) per batch
