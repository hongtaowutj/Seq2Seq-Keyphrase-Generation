...
class Decoding():
	...
	def __init__(self, encoder_model, decoder_model, indices_words, words_indices, enc_in_seq,  states, attentions, decoder_length, rnn_dim,  beam_width, num_hypotheses, filepath, filename):
	...
	def beam_decoder(self, texts, s, att):
	...
	def beam_generator(self):
		...
		for i in range(len(self.enc_in_seq)):
			...
			encoder_output = self.encoder_model.predict(text)
			states_value = self.states[i:i+1]
			attention_weight = self.attentions[i:i+1]

			start_id = self.words_indices['<start>']

			next_fringe = [Node(parent=None, state=states_value, value=start_id, cost=0.0, extras=attention_weight)]
			hypotheses = []
			...
			for t_ in range(self.decoder_length+1):
				...
				for n in fringe:
					...
					predicted_prob, attention_t, state_t = self.decoder_model.predict([dec_input] + [encoder_output] + [states_value] )
				...
				attentions = attentions.reshape((attentions.shape[0],attentions.shape[2]))
				...
				next_fringe = []
				for Y_tn, prob_tn, state_tn, att_tn, n in zip(predicted_indices, probs, states, attentions, fringe):
					...
					for y_tn, logprob_tn in zip(Y_tn, logprob_tn):
						n_new = Node(parent=n, state=state_tn, value=y_tn, cost=logprob_tn, extras=att_tn)
