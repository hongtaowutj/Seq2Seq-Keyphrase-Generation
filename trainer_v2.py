...
from models.seq2seq_sampled_softmax_v2 import SampledSoftmax
...
def trainer(params):
	...
	try:
		...
		#glove_conn = DataConnector(glove_path, 'glove.6B.100d.txt', data=None)
		glove_conn = DataConnector(glove_path, glove_name, data=None)
		embedding, pretrained_words = glove_conn.load_embedding(indices_words, words_indices, 100)
		print("indices_words size: %s"%len(indices_words))
		print("embedding shape: %s"%str(embedding.shape))
		print("pretrained_words size: %s"%len(pretrained_words))

		glove_embed = DataConnector(result_path, 'glove_embedding_%s'%glove_name, embedding)
		glove_embed.save_pickle()

		glove_pretrained_words = DataConnector(result_path, 'glove_pretrained_words_%s'%glove_name, pretrained_words)
		glove_pretrained_words.save_pickle()


		oov_embedding, oov = glove_conn.create_oov_embedding(pretrained_words, indices_words, words_indices, 100)

		print("oov_embedding shape: %s"%str(oov_embedding.shape))
		print("oov size: %s"%len(oov))
		print("oov[:10]: %s"%str(oov[:10]))

		oov_embed = DataConnector(result_path, 'oov_embedding_%s'%glove_name, oov_embedding)
		oov_embed.save_pickle()

		oov = DataConnector(result_path, 'oov_%s'%glove_name, oov)
		oov.save_pickle()

		t1 = time.time()
		print("matrix embedding preparation is done in %.3fsec" % (t1 - t0))
		sys.stdout.flush()
	...
	sampled_softmax = SampledSoftmax(encoder_length, decoder_length, embedding_dim, birnn_dim, rnn_dim, vocab_size, num_samples, result_path, file_name, batch_train_iter, batch_val_iter, batch_size, steps_epoch, val_steps, epoch)
	...
	sampled_softmax.train_sampled_softmax(pretrained_embedding, oov_embedding)
	...
	t0 = time.time()
	print("Training model with approximate softmax and double embeddings...")
	sys.stdout.flush()
