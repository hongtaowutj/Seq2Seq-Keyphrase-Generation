...
class ConvAttentionSampled():
	...
	def train_conv_att_sampled_softmax(self):
		...
		self.repeator = RepeatVector(self.in_gru_enc, name='repeator_att')
	...
	def compile_(self):
		...
		print("\n--- Attentive Seq2Seq (Conv + Bidirectional-GRU) with sampled softmax): trainable model --- \n")
	...
	def eval_conv_att_sampled_softmax(self, stored_weights):
	...
	def predict_conv_att_sampled_softmax(self, stored_weights):
	...
	def create_decoder_model(self):
		...
		enc_out = Input(shape=(self.in_gru_enc, self.rnn_dim))
		...
		decoder_states = [s]
		decoder_model = Model([in_decoder] + [enc_out] + [in_state_decoder] + [in_label], [softmax_prob] + [alphas] + decoder_states)
