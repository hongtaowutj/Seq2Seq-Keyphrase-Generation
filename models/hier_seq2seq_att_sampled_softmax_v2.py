...
from keras.layers import Input, Embedding, Dropout, Activation, Add
...
class HierarchyAttSampledSoftmax():
	...
	def train_hier_att_sampled_softmax(self, pretrained_embedding, oov_embedding):
	...
	def compile_(self):
		...
		print("\n--- Hierarchical Attentive Seq2Seq (CNN + Bidirectional-GRU) with sampled softmax): trainable model --- \n")
	...
	def create_decoder_model(self):
		...
		decoder_states = [s]
		decoder_model = Model([self.in_decoder] + [doc_enc_out] + [in_state_decoder] + [in_label], [softmax_prob] + [alphas] + decoder_states)
