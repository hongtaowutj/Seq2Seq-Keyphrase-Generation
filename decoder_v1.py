...
def decoder(params):
	...
	'''
	indices_words_connector = DataConnector(preprocessed_v2, 'all_indices_words.pkl', data=None)
	indices_words_connector.read_pickle()
	indices_words = indices_words_connector.read_file

	words_indices_connector = DataConnector(preprocessed_v2, 'all_words_indices.pkl', data=None)
	...
	sampled_softmax = SampledSoftmax(encoder_length=encoder_length, decoder_length=decoder_length, embedding_dim=embedding_dim, birnn_dim=birnn_dim, rnn_dim=rnn_dim, vocab_size=vocab_size, num_samples=num_samples, filepath=result_path, filename=file_name, batch_train_iter=None, batch_val_iter=None, batch_size=None, steps_epoch=None, val_steps=None, epochs=None)

	# skeleton of model architecture
	sampled_softmax.train_sampled_softmax()
	
	predict_softmax_model=sampled_softmax.predict_sampled_softmax(weights)
	...
	greedy_keyphrases = inference_mode.greedy_decoder(X_test, y_dummy_test)

	greedy_decode_connector = DataConnector(result_path, 'greedy_kp-%s.npy'%(file_name), greedy_keyphrases)
	greedy_decode_connector.save_numpys()

	t1_0 = time.time()
	print("Greedy decoding is done in %.3fsec" % (t1_0 - t0_0))
	sys.stdout.flush()
	...
	beam_keyphrases = inference_mode.beam_decoder(X_test[:500], y_dummy_test[:500])
